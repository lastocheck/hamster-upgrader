...
const loadFrom = process.argv.slice(2)[1];
const cardsToUpgade = process.argv.slice(2)[0];

(async () => {
    const userDataDir = path.join(__dirname, 'custom_profile'); // Path to your custom profile directory
    const extensionPath = path.join(__dirname, 'Violentmonkey-Chrome-Web-Store'); // Path to the unpacked extension

    // Launch browser with a custom browser profile and load the storage state
    const browser = await chromium.launchPersistentContext(userDataDir, {
        headless: false,
        args: [`--disable-extensions-except=${extensionPath}`, `--load-extension=${extensionPath}`]
    });

    // Load the saved context state from storageState.json
    const storageStatePath = './storageState.json'; // Path to your storage state file
    ...
    if (!loadFrom) await page.waitForTimeout(8000000);
    ...
    await page.goto('https://web.telegram.org/k/#@hamster_kombat_bot');

    // await page.getByText('Играть в 1 клик').click();
    // const button = await page.waitForSelector('//span[contains(text(), "Играть в 1 клик")][last()]');
    const button = await page.waitForSelector('.new-message-bot-commands-view');
    await button.click();
    ...
    await iframe.locator('css=a.header-info-settings').click();
    await iframe.getByText('Сменить язык').click();
    await iframe.getByText('English (en)').click();
...
async function upgradeCards(iframe, bestCards) {
    for (const card of bestCards) {
        ...
        await iframe.getByText('Go ahead').click();
...
async function processTab(tab, cards, iframe, selectors) {
    ...
    for (const upgradeItem of upgradeItems) {
        ...
        if (!cardElementsExist) {
            await iframe.locator('css=div.bottom-sheet-close').click();
            console.log(`couldn't find price or profit on card ${cardName}`);
            continue;
        } else {
            cardElementsExist = false;
            retryCount = 0;

            while (retryCount < 5) {
                cardElementsExist = await iframe.evaluate(() => {
                    return document.querySelector('div.upgrade-buy .upgrade-buy-info .price-value') !== null;
                });
...
function convertPrice(price) {
    if (price.includes('.')) return parseFloat(price.replace(/\s/g, ''));
